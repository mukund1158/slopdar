...
**What it is:** While the scan animation is running, the player is asked to call the result before the radar does. Guess right and you earn radar accuracy points.

**How it works:**
1. User submits a URL as normal. The scanning screen appears.
2. During the animation (the scan takes a few seconds anyway, so this fills dead time), a prompt appears: "Call it before the radar does" with two buttons: "Slop" and "Built".
   - "Slop" means the user predicts tier Vibe-Coded or Pure Slop (score 51 to 100).
   - "Built" means Hand-Crafted or Suspiciously Clean (score 0 to 50).
3. When the result arrives, the result screen shows a small verdict next to the score ring:
   - Correct: "Good radar. Streak: 4"
   - Wrong: "Radar jammed. The receipts say otherwise."
4. A running accuracy stat (correct guesses / total guesses) and current streak are stored in the browser (localStorage). No account needed.

**Data and code needed:** frontend only. New state in `SlopdarApp.tsx`, a localStorage record like `{ guesses: 12, correct: 9, streak: 4 }`. No database change, no API change.

**Edge cases:** if the user does not guess in time, skip silently. If the scan fails (unreachable site), discard the guess.
...
These need login. The `User` and `SavedSite` models already exist in the Prisma schema, and Google sign-in via Auth.js was already the plan, so this phase starts by wiring that up.
